...
def generate_time_series(duration=80, sampling_rate=20, num_peaks=3, noise_std=0.1, noise_prob=0.1):
...
# Apply PCA to reduce the point cloud to 2D
pca = PCA(n_components=2)
projected = pca.fit_transform(clean_point_cloud)

plt.figure(figsize=(8, 6))
plt.scatter(projected[:, 0], projected[:, 1], alpha=0.6, label="PCA Projection")
plt.title("PCA Projection of Time-Delay Embedded Point Cloud")
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.legend()
plt.grid()
...
tsne_proj = tsne.fit_transform(clean_point_cloud)

plt.figure(figsize=(8, 6))
plt.scatter(tsne_proj[:, 0], tsne_proj[:, 1], alpha=0.6, label="t-SNE Projection")
plt.title("t-SNE Projection of Time-Delay Embedded Point Cloud")
plt.xlabel("Dim 1")
plt.ylabel("Dim 2")
plt.legend()
plt.grid()
